...
local BLADE_HUSK_LOOP_EVENT = "wwise/events/weapon/play_transonic_blades_idle_loop_husk"
...
local HAMMER_HUSK_LOOP_EVENT = "wwise/events/weapon/play_thunder_hammer_powered_loop_husk"
...
local SKULL_HUM_EVENT      = "wwise/events/player/play_cryptic_servitor_loop"
local SKULL_HUM_HUSK_EVENT = "wwise/events/player/play_cryptic_servitor_loop_husk"

local ARC_MAUL_IDLE_EVENT         = "wwise/events/weapon/play_arc_maul_idle_loop"
local ARC_MAUL_IDLE_HUSK_EVENT    = "wwise/events/weapon/play_arc_maul_idle_loop_husk"
local ARC_MAUL_SPECIAL_EVENT      = "wwise/events/weapon/play_arc_maul_1h_special_loop"
local ARC_MAUL_SPECIAL_HUSK_EVENT = "wwise/events/weapon/play_arc_maul_1h_special_loop_husk"

local GALVANIC_IDLE_EVENT      = "wwise/events/weapon/play_galvanic_rifle_cylinder_idle_loop"
local GALVANIC_IDLE_HUSK_EVENT = "wwise/events/weapon/play_galvanic_rifle_cylinder_idle_loop_husk"

local suppress_shield          = true
local suppress_acd             = true
local suppress_transonic_blade = true
local suppress_arc_maul        = true
local suppress_galvanic_rifle  = true
local suppress_chordclaw       = true
local suppress_servo_skull_hum = true
...
local function refresh_settings()
	...
	suppress_arc_maul        = mod:get("suppress_arc_maul")
	suppress_galvanic_rifle  = mod:get("suppress_galvanic_rifle")
	suppress_chordclaw       = mod:get("suppress_chordclaw")
	suppress_servo_skull_hum = mod:get("suppress_servo_skull_hum")
...
mod:hook(WwiseWorld, "trigger_resource_event", function(func, wwise_world, event_name, ...)
	...
	if suppress_transonic_blade and (event_name == BLADE_LOOP_EVENT or event_name == BLADE_HUSK_LOOP_EVENT) then
		return
	end

	if suppress_arc_maul and (event_name == ARC_MAUL_IDLE_EVENT or event_name == ARC_MAUL_IDLE_HUSK_EVENT
		or event_name == ARC_MAUL_SPECIAL_EVENT or event_name == ARC_MAUL_SPECIAL_HUSK_EVENT) then
		return
	end

	if suppress_galvanic_rifle and (event_name == GALVANIC_IDLE_EVENT or event_name == GALVANIC_IDLE_HUSK_EVENT) then
		return
	end

	if suppress_servo_skull_hum and (event_name == SKULL_HUM_EVENT or event_name == SKULL_HUM_HUSK_EVENT) then
		return
	end
